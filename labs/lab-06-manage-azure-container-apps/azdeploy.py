...
rg = "aca-lab06"  # Resource Group name
location = "eastasia"   # Azure region for the resources
...
def create_acr_and_build_image(acr_name: str) -> bool:
    ...
    print("Logging into ACR with local Docker...")
    if not run_quiet("ACR Login", ["az", "acr", "login", "--name", acr_name]):
        return False
        
    print("Building image locally...")
    if not run_quiet("Docker Build", ["docker", "build", "--platform", "linux/amd64", "-t", f"{acr_name}.azurecr.io/{CONTAINER_IMAGE}", "api/"]):
        return False
        
    print("Pushing image to ACR...")
    if not run_quiet("Docker Push", ["docker", "push", f"{acr_name}.azurecr.io/{CONTAINER_IMAGE}"]):
        return False
...
def deploy_container_app_and_configure_secrets(acr_name: str, aca_env: str) -> bool:
    ...
    if not app_exists:
        print("Enabling ACR Admin user to bypass Express Environment limitations...")
        az_query(["az", "acr", "update", "--name", acr_name, "--admin-enabled", "true"])
        
        acr_password = az_query(
