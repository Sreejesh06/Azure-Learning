...
rg = "rg-ai-aks-lab"  # Resource Group name
...
def provision_foundry_resources(foundry_resource: str) -> bool:
    ...
    if not deployment:
        print(f"Bypassing model deployment for {FOUNDRY_MODEL_NAME} due to subscription restrictions...")
        # if not run_quiet(
        ...
        # ):
        #     return False
        print("Model deployed successfully (MOCKED)")
    else:
...
def build_and_push_image(acr_name: str) -> bool:
    ...
    if not run_quiet("Login to ACR", ["az", "acr", "login", "--name", acr_name]):
        return False

    image_tag = f"{acr_server}/{API_IMAGE_NAME}:latest"
    print(f"Building Docker image locally (linux/amd64) and tagging as {image_tag}...")
    if not run_quiet(
        "Docker build",
        [
            "docker", "build",
    ...
    if not run_quiet("Docker push", ["docker", "push", image_tag]):
        return False
