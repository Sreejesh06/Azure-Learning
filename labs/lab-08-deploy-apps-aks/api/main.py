...
async def streaming_inference(request: Request):
    ...
    async def event_generator():
        try:
            ...
            mock_message = f"[MOCKED] This is a mock streamed AI response from Kubernetes for your prompt: '{prompt}'"
...
async def call_foundry_inference(
    ...
    try:
        # MOCKED RESPONSE to bypass Azure OpenAI restrictions
        # client = get_openai_client()
        # response = client.chat.completions.create(
        #     model=FOUNDRY_DEPLOYMENT,
        #     messages=[{"role": "user", "content": prompt}],
        ...
        return {
            "choices": [{
                "message": {
                    "role": "assistant",
                    "content": f"[MOCKED] This is a mock AI response from Kubernetes for your prompt: '{prompt}'"
                }
            }]
        }
    except Exception as e:
