...
rg = "rg-aca-scale-eastasia"  # Resource Group name
...
def _derived_names(user_object_id: str) -> tuple[str, str]:
    ...
    return f"acr2{user_hash}", f"aca2-env-{user_hash}"
...
def create_acr_and_build_image(acr_name: str) -> bool:
    ...
    print()
    print("Building and pushing container image locally using Docker...")
    ...
    if not run_quiet(
        ...
        ["docker", "push", container_image_fqdn],
    ):
        return False
...
def create_container_app(acr_name: str, aca_env: str) -> bool:
    ...
    if app_exists:
        ...
        acr_username = az_query(["az", "acr", "credential", "show", "--name", acr_name, "--query", "username", "-o", "tsv"])
        acr_password = az_query(["az", "acr", "credential", "show", "--name", acr_name, "--query", "passwords[0].value", "-o", "tsv"])
    ...
    # Role assignment skipped: Using admin credentials for ACR per student rules.
