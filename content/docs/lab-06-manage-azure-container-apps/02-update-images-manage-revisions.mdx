...
> **Note**: The examples in this unit are patterns. Validate flags and required fields against the current Azure CLI and Azure Container Apps documentation before using these commands in automation.
...
```mermaid
graph TD
    UpdateCMD["az containerapp update<br/>--image new_model:v2"]
    
    subgraph ACA [Container App]
        Rev1["Revision 1<br/>(Old, Still active)"]
        Rev2["Revision 2<br/>(New, Spinning up)"]
    end
...
```mermaid
graph TD
    subgraph Single Revision Mode
        Traffic1((100% Traffic)) --> Rev2_Single["Revision 2 (Active)"]
        Rev1_Single["Revision 1 (Shut down)"]
    end

    subgraph Multiple Revision Mode
        Traffic2((Traffic)) -- "90%" --> Rev1_Multi["Revision 1 (Active)"]
        Traffic2 -- "10% Canary" --> Rev2_Multi["Revision 2 (Active)"]
    end
```
...
- [Azure Container Apps application lifecycle management](https://learn.microsoft.com/en-us/azure/container-apps/application-lifecycle-management)
- [Azure CLI: az containerapp](https://learn.microsoft.com/en-us/cli/azure/containerapp)
- [Azure CLI: az containerapp revision](https://learn.microsoft.com/en-us/cli/azure/containerapp/revision)
