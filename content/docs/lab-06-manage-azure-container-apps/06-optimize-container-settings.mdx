...
> **Note**: Resource and scaling options depend on your environment configuration and your workload type. Use these examples as patterns, and validate constraints and defaults in the current Azure Container Apps documentation.
...
### CPU vs. RAM (The Desk Worker Analogy)

Think of your container as a **worker sitting at a desk**:
- **CPU (Processing Power) = How fast the worker's brain is.** If you give the container low CPU, the worker is slow. The job gets done, but it takes forever. In cloud terms, this is **"Throttling"**. Your app stays alive, but it gets very slow and users experience lag.
- **RAM (Memory) = The size of the desk.** If the worker needs to open a giant textbook (like a 2GB AI model) to do their job, they need a big desk. If you give them a tiny desk, the book falls on the floor, the worker gets frustrated, and quits instantly. In cloud terms, this is an **"OOM-Kill" (Out Of Memory Kill)**. The app completely crashes and restarts.
...
### The Coffee Shop Analogy for Scaling

- **Synchronous API (The Cashier)**: The user clicks a button and stares at a loading spinner waiting for an answer. You **cannot** afford a 30-second "Cold Start" while the server wakes up. You must keep at least 1 replica awake 24/7 (`min-replicas=1`). It costs money to keep it awake, but you guarantee an instant response.
- **Background Worker (The Dishwasher)**: A user uploads a document to be processed in the background. The document goes into a queue. Nobody is actively staring at a loading screen. You can set the worker to go to sleep completely (`min-replicas=0`). If the queue is empty, you pay $0.00. When a document arrives, the server takes 30 seconds to wake up, does the job, and goes back to sleep.
...
Optimization is iterative. You use logs and metrics to identify bottlenecks, then you change one variable at a time. This approach reduces the risk of masking the root cause and makes it easier to understand which change improved throughput or latency.
...
- [Azure Container Apps scale applications](https://learn.microsoft.com/en-us/azure/container-apps/scale-app)
- [Azure Container Apps manage applications](https://learn.microsoft.com/en-us/azure/container-apps/overview)
